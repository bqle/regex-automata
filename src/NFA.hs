...
instance (Show transitionT, Show acceptingT) 
  => Show (Automaton transitionT acceptingT) where
...
exploreEpsilons transition states = 
  let
    immediateFrontier = nub $ concatMap 
      (makeTransition transition [] epsilon) states
  in 
...
alphabet ls = 
  let
    ...
    in 
    Aut 0 initSt transitions accSt
...
alternate nfa1 nfa2 =  
    let 
      ...
      Aut 0 newInitSt newTransitions newAccSt 
    where 
      Aut uuid1 init1 trans1 accept1 = attachUUID nfa1 
      Aut uuid2 init2 trans2 accept2 = attachUUID nfa2 
...
kleene nfa = 
  ...
  Aut 0 newInitSt (unionTransitions newTransitions trans) newAccSt
